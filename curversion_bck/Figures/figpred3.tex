...
\pgfplotsset{
  ...
  \def\DISX{../data/figfiles/MFXcodes.csv}
  \def\DISM{ntb/MFmatch.csv}
  \def\DISM{../data/figfiles/MFcodes.csv}

  \def\DISMM{ntb/male.csv}
  \def\DISFF{ntb/female.csv}
  \def\DISMM{../data/figfiles/Mcodes.csv}
  \def\DISFF{../data/figfiles/Fcodes.csv}
  ...
  \node [anchor=north west,align=left] (M) at ([yshift=-.225in]R.south west) {
   ...
   nodes near coords,
   every node near coord/.append style={anchor=west,align=left, text width=3in,font=\sffamily\rm\fontsize{6}{8}\selectfont,text=darkgray,
        shift={(axis direction cs:0.008-.03*\rawx*\coordindex,0)}},
   point meta=explicit symbolic,ylabel={ICD9 codes}, xtick ={-0.04,0,0.04}
        ] 
