...
\def\HBLK{gray}
  ...
  \def\COMPA{../data/figfiles/logodds_s_Mcodes.csv}
  \def\COMPB{../data/figfiles/logodds_s_Fcodes.csv}
  %\def\COMPA{../data/figfiles/age_3_Comp_logodds_Mcodes.csv}
  %\def\COMPB{../data/figfiles/age_3_Comp_logodds_Fcodes.csv}
  \def\COMPMF{../data/figfiles/logodds_c_MFComp.csv}
  \def\COMPMFA{../data/figfiles/age_3_logodds_MFComp.csv}
...
\clip (.85in,0.230in) rectangle (7.8in,-9.05in);
...
% }
  ...
  % xmax=0.03,xmin=-0.03,
    ...
    \end{tikzpicture}};
      ...
      \node [anchor=south west,align=left] (B) at ([xshift=-1.7in]A.south east) {
        \begin{tikzpicture}[text=\TEXTCOL]
%
%   \def\basis{1}
%   \pgfplotsset
...
% }
  ...
  % xmax=0.03,xmin=-0.03,
      ...
      \end{axis}
    \end{tikzpicture}};
   \node[fill=white,anchor=north west] (T) at ([xshift=-2in,yshift=0.225in]B.north east) {
...
% }
          ...
          \begin{groupplot}[group style={group name=A,group size= 2 by 1,horizontal sep=0in,},legend style={anchor=east,at={(0.5,1.05)},inner sep=3pt,draw=none,fill=white,fill opacity=.85,align=right,text opacity=1,font=\bf\sffamily\fontsize{8}{9}\selectfont},axis line style={lightgray, opacity=0, thin},%
        enlargelimits=false,
        anchor=north west,
        height=2.6in,
        width=\WDTX,
  ...
  \node[anchor=south west,align=left] (LB) at ([xshift=3in,yshift=0in]$(LA.north west)!(B.north)!(LA.south west)$) {{\LARGE B.} Female };
