...
\def\COLBL{black}
  ...
  \def\COMPA{../data/figfiles/logodds_patternImmun.csv}
  \def\COMPB{../data/figfiles/logodds_patternnfect.csv}
  \def\COMPC{../data/figfiles/logodds_patternRespira.csv}
  \def\MTYP{MImmun}
  \def\FTYP{FImmun}
  ...
  \def\BWIDTH{8pt}
  \def\BWIDTHB{7pt}
  \clip (.75in,0.25in) rectangle (7.5in,-8in);
...
% }
  ...
  % xmax=0.03,xmin=-0.03,
    ...
    \end{tikzpicture}};
      ...
      \node [anchor=south west,align=left] (B) at ([xshift=-1.75in]A.south east) {
        \begin{tikzpicture}[text=\TEXTCOL]
%
%   \def\basis{1}
%   \pgfplotsset
...
% }
 ...
 \node[anchor=south west,align=left] (LA) at ([xshift=1in,yshift=0in]A.north west) {{\LARGE A.} Endocrine Nutritional  Metabolic  And Immunity Dis.
  ...
  \node[anchor=south west,align=left] (LB) at ([xshift=3.5in,yshift=0in]$(LA.north west)!(C.north)!(LA.south west)$) {{\LARGE B.} Respiratory Disorders };

  \node[anchor=south west,align=left] (LC) at ([xshift=3.5in,yshift=-3.75in]$(LA.north west)!(C.north)!(LA.south west)$) {{\LARGE C.} Infectious And Parasitic Diseases
 };
\end{tikzpicture}
