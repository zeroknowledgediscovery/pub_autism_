...
\def\COLBL{black}
  ...
  \def\COMPA{\DATA/figfiles/age_3_logodds_patternervous.csv}
  \def\COMPC{\DATA/figfiles/age_3_logodds_patterndefined.csv}
  \def\MTYP{Mervous}
  \def\FTYP{Fervous}
...
% }
  ...
  % xmax=0.03,xmin=-0.03,
        ...
        y expr=\coordindex,
        x=pn, discard if not={typ}{Mdefined}
        ] {\COMPC};
        \addplot[draw=\FXCOL,fill=\FXCOL,xbar,area legend,opacity=\OPC,text opacity=1] table [ 
        y expr=\coordindex,
        x=pn, discard if not={typ}{Fdefined}
        ] {\COMPC};
...
%         ] {\COMPC};
  ...
  \node[anchor=south west,align=left] (LA) at ([xshift=1in,yshift=0in]A.north west) {{\LARGE A.} Nervous System \& Sense Organs };
  \node[anchor=south west,align=left] (LB) at ([xshift=3.5in,yshift=0in]$(LA.north west)!(A.north)!(LA.south west)$) {{\LARGE B.} Ill-defined Symptoms \& Disorders };
