...
\def\COLBL{black}
  ...
  \def\COMPA{\DATA/figfiles/age_3_logodds_patternCongenital.csv}
  \def\COMPC{\DATA/figfiles/age_3_logodds_patternMusculo.csv}
  \def\MTYP{MCongenital}
  \def\FTYP{FCongenital}
  ...
  \def\BWIDTH{8pt}
    \def\BWIDTHB{9pt}
  \def\BWIDTHC{7pt}
  \clip (.85in,0.25in) rectangle (7.6in,-6in);
...
% }
  ...
  % xmax=0.03,xmin=-0.03,
    ...
    \end{tikzpicture}};
      ...
      \node [anchor=north west,align=left] (B) at ([xshift=-1.65in,yshift=0in]A.north east) {
        \begin{tikzpicture}[text=\TEXTCOL]
%
%   \def\basis{1}
%   \pgfplotsset
...
% }
  ...
  % xmax=0.03,xmin=-0.03,
        ...
        y expr=\coordindex,
        x=pn, discard if not={typ}{MMusculo}
        ] {\COMPC};
        \addplot[draw=\FXCOL,fill=\FXCOL,xbar,area legend,opacity=\OPC,text opacity=1] table [ 
        y expr=\coordindex,
        x=pn, discard if not={typ}{FMusculo}
        ] {\COMPC};
...
%         ] {\COMPC};
  ...
  \node[anchor=south west,align=left] (LA) at ([xshift=1in,yshift=0in]A.north west) {{\LARGE A.} Congenital Disorders };
  \node[anchor=south west,align=left] (LB) at ([xshift=3.5in,yshift=0in]$(LA.north west)!(A.north)!(LA.south west)$) {{\LARGE B.} Musculoskeletal \& Connective Tissue Disorders };
