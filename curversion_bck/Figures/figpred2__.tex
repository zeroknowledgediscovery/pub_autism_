...
\pgfdeclarepatternformonly{north east lines wide}%
  ...
  \def\auctimeV{../validation/AUC_STATS.csv}
  ...
  \node[anchor=center] (N) at (A0.south)
  {
   ...
   \begin{axis} [,legend cell align={left},
    ...
    enlarge x limits=0.07,axis line style={\AXISCOL, opacity=1,ultra  thick, rounded corners=0pt}, y tick label style={/pgf/number format/fixed,/pgf/number format/precision=3,/pgf/number format/fixed zerofill,
     ...
     },] (axis cs:125,\pgfkeysvalueof{/pgfplots/ymin}) -- (axis cs:125,\pgfkeysvalueof{/pgfplots/ymax}) -- (axis cs:100,\pgfkeysvalueof{/pgfplots/ymax}) -- (axis cs:100,\pgfkeysvalueof{/pgfplots/ymin}) --cycle;
    ...
    },]table[col sep=comma,x=n_weeks, y=auc, discard if not={gender}{M}
    ]
    {\auctimeV};
...
%
    ...
    },]table[col sep=comma,x=n_weeks, y=auc, discard if not={gender}{F}]
    {\auctimeV};
...
\end{axis}\end{tikzpicture}
  ...
  \node[anchor=north west] (A) at ([xshift=0in]N.north east) {
...
text width=1.1in, anchor=east, yshift=0in,xshift=-.045in,text=\TEXTCOL},
    ...
    \addlegendentry{Female Treatment}
  \end{groupplot}
\end{tikzpicture}
};
...
\node[anchor=north west] (B) at ([yshift=-.275in]N.south west)
{
  \begin{tikzpicture}[]
   ...
   \addplot[forget plot,smooth, draw=none,,name path=AA]table[col sep=comma,x=time, y=UcontrolM]
    ...
    {\riskfile};
    \addlegendentry{Males Treatment};
   ...
   \addplot[forget plot,smooth, draw=none,,name path=Ax,]table[col sep=comma,x=time, y=UcontrolF]
    ...
    {\riskfile};
    \addlegendentry{Females Treatment};
...
\def\OPC{.2}
   ...
   \addplot[smooth, draw=lightgray, very thick, fill=Lavender, fill opacity=\OPC,,,name path=A,area legend,postaction={
    ...
    \addlegendentry{Male (Truven)}
     ...
     \draw [CadetBlue4,,semithick] (axis cs:135,\pgfkeysvalueof{/pgfplots/ymin}) -- (axis cs:135,\pgfkeysvalueof{/pgfplots/ymax}) node [pos=1.0,midway,left,yshift=-0.15in,xshift=-.025in,fill=white] {135 weeks};
       \end{axis}
      \end{tikzpicture}
    };


      \node [anchor=north west] (E) at ([xshift=0in,yshift=-.5in]D.south west)
  {
    \begin{tikzpicture}
      \begin{axis}[legend cell align=left, legend columns=2,     legend style={,anchor=east,at={(1,1.1)},
        ...
        xlabel style={yshift=0.05in,text=\TEXTCOL},ylabel style={yshift=-0.1in,xshift=-.7in,text=\TEXTCOL},x tick label style={,text=\TEXTCOL,xshift=.1in,yshift=-.05in,/pgf/number format/fixed,/pgf/number format/precision=2,/pgf/number format/fixed zerofill,
          /pgf/number format/1000 sep = %\thinspace
...
\def\OPC{.6}
\def\SCALER{0.01}
\node [anchor=north west] (F) at ([xshift=0.1in,yshift=-.15in]E.north east)
  {
    \begin{tikzpicture}
...
%
  ...
  \end{axis}
   ...
   \node[anchor=north west,align=left] (LB) at ($(LA.north west)!(A.north west)!(LA.north east)$) {{\LARGE B.} Model Complexity for Control \& Treatment Groups  };
  \node[anchor=south west,align=left] (LB1) at ($(LA.north west)!(B.north west)!(LA.south west)$) {{\LARGE C.} Average risk over age   };
  \node[anchor=south west,align=left] (LB2) at ($(LB1.south west)!(LB.west)!(LB1.south east)$) {{\LARGE D.} Time to Clinical Diagnosis after Relative Risk $> 90\%$   };
  \node[anchor=south west,align=left] (LB3) at ([xshift=-.6in]$(LB2.south west)!(E.north west)!(LB2.north west)$) {{\LARGE E.} Effect of Early Exposure \\($<50$ weeks)  to  Diseases  };

 \node[anchor=north west,align=left] (LB3) at ([xshift=0.2in]$(LB3.north west)!(F.north west)!(LB3.north east)$) {{\LARGE F.} Risk Progression Example with disorders\\color-coded  in diagnostic history\\(Truven database, clinical diagnosis at $148$ wk) };
