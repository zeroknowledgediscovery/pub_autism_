...
% while othetr ML methods have been attempted, leveraging videoa and sophisticated analysis of questioinarries, there is a key difference in the results. All those approaches do not velerage comorbidity. In some sense such approaches model the physician, no new insight is often possible fro such approaches. We othe otherhand model the disdes processes and it sfootprint in different functions systems that manifest as subtle difference in comorbidity patterns. Thus, we model the diease more directly, and there is clear opportuniuty to gaining new insight into teh pathopsyiology of autism.
