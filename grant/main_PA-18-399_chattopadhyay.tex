...
\input{../../../publications/pub_autism_/custom_defs.tex}
...
\begin{wrapfigure}[8]{r}{2.5in}
 ...
 \rowcolor{lightgray} ADOS / ADOS-2 & Autism Diagnostic Observation Schedule \\
  \rowcolor{lightgray!50}ABA  & Applied Behavior Analysis \\
...
\paragraph*{Paradigm Shift in ASD Screening} Despite extensive documentation of co-morbidities,   a risk estimator that makes reliable predictions for  individuals | based purely on co-morbidity patterns |  has never been reported to  our knowledge. The sparsity of  diagnostic codes in individuals,  the  absence of physiological disorders  that would consistently signal the eventual emergence of ASD symptoms, combined with the heterogeneity of ASD presentation,  make such an endeavor challenging. In this study we leverage our preliminary work on  the  formulation of a \textit{first-of-its-kind}  framework to make predictions 
based on models of statistically curated patterns of diagnostic code sequences automatically learned from  sufficiently large databases of electronic health records (EHR), that  achieves an out-of-sample AUC exceeding  $80\%$ for  either sex from just over 2 years of age. The machine learning tools that make this possible are also fundamentally novel, designed to address the specific issues in handling sparse, noisy categorical diagnostic sequences. {\color{Red1} Most recent ML advances aim to model the physician, nothing ne wis learnt}
 %###########################################################
%###########################################################
%###########################################################
\subsection{Approach}
We leverage the \ZERO methodology from our preliminary studies, towards prospective  application in a primary care setting. We describe the formulation of the \ZERO score with a brief description of our preliminary results.
...
We extracted histories of patients within the age of $0-5$ years, and excluded \begin{wrapfigure}[37]{l}{2.4in}
   \tikzexternaldisable
  ...
  \centering 
   ...
   \vspace{-15pt}
    ...
    \captionN{Variation of Inferred Risk. Panel A illustrates AUC achieved as a function of
      ...
      % Panel F illustrates  how inferred models differ between the control vs. the \treatment cohorts. On average, models get less complex, implying the exposures get more statistically independent.
    }\label{fig2}
       \vspace{-15pt}
...
\end{wraptable}  
%####################################
We plot the absolute or raw risk over time for males and females for the out-of-sample control and \treatment cohorts in Fig.~\ref{fig2}B. % We see that while the risk for the control cohort remains more or less stable,   that for the \treatment cohort rapidly increases.
Notably, in these risk plots, averaged over the population,   we see disambiguation  early, right from $50$ weeks. Also, we see a saturation of the risk after $150$ weeks, which corresponds to the median diagnosis age in the database (approx. 150 weeks). %Thus, if a child is not diagnosed up to that age, then the  risk  falls, since the probability of a diagnosis in the population starts to go down after this age.
...
%     \node[anchor=west] (B) at (A.east) {\includegraphics[height=1.5in,angle=0]{Figures/bars2}};
%   \end{tikzpicture}
%   \vspace{-10pt}
...
%
%We will investigate  discrimination between ASD and  unrelated psychiatric phenotypes.
%
...
\begin{wrapfigure}[24]{l}{2.5in}
  ...
  \hspace{-10pt}\includegraphics[width=2.9in]{Figures/flow}
  \vspace{-35pt}
  
  \caption{Patient processing}\label{figflow}
\end{wrapfigure}
